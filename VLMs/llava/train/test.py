...
def test(attn_implementation=None):
    ...
    question_file = data_args.question_file
    ...
    if model_args.vision_tower is not None:
        ...
        with open(os.path.join(training_args.output_dir, "inference_config.json"), "w") as f:
            json.dump(inference_cfg, f, indent=4)
        
        print(f"Saving inference config to {os.path.join(training_args.output_dir, 'inference_config.json')}")
        print(inference_cfg)
        ...
        for line in tqdm(questions):
            ...
            # prompt = 'A chat between a curious human and an artificial intelligence assistant. The assistant provides a helpful and concise answer, selecting only one option from the list provided. USER:'
            # prompt = prompt.replace(", detailed, and polite answers to the human's questions", " and concise answer, selecting only one option from the list provided")
            # print('PROMPT: ', prompt)
